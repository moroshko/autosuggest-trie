...
describe('intersectionWithLimit', () => {
  describe('when limit is not specified', () => {
    it('should return [] if one of the arrays is empty', () => {
      // prettier-ignore
      expect(intersectionWithLimit([[6, 1, 3, 2, 4], [], [99, 3, 11]]))
        .to.be.empty;
    });

    it('should return [] if intersection is empty', () => {
      expect(
        intersectionWithLimit([
          [16, 1, 125, 3, 4],
          [100, 2, 5, 189, 6, 16, 0],
          [189, 0, 3, 125, 4, 6]
        ])
    ...
    it('should return the array if only one array is given', () => {
      // prettier-ignore
      expect(intersectionWithLimit([[16, 1, 4, 3, 125]]))
  ...
  describe('when limit is specified', () => {
    it('should return [] if one of the arrays is empty', () => {
      // prettier-ignore
      expect(intersectionWithLimit([[4, 1, 3], [], [0, 11, 3, 4, 7]], 2))
        .to.be.empty;
    });
    ...
    it('should return the start of the array single array is given', () => {
      // prettier-ignore
      expect(intersectionWithLimit([[125, 88, 9, 3, 17, 2]], 3))
        .to.deep.equal([125, 88, 9]);
    });
